...
TEACHING_EXPLANATION_SYSTEM_PROMPT = """You are a math teacher and educational coach.
...
TEACHING_EXPLANATION_FEW_SHOT = {
    ...
    "output": {
        "explanation": "本题是典型的闭区间函数最值问题。核心思路是：对于闭区间上的连续函数，最大值和最小值只可能出现在两类位置——区间端点，或者导数为零的驻点。因此我们只需要三步：(1) 求导找到驻点；(2) 把驻点和端点都代入原函数算出函数值；(3) 比较这些函数值，最大的就是最大值，最小的就是最小值。这道题里驻点是 x=-1 和 x=1，加上端点 x=-2 和 x=2，四个点代入后得到函数值 3 和 -1，所以最大值是 3，最小值是 -1。",
        "key_points": [
        ...
        "practice_questions": [
            ...
            "求函数 f(x)=x+1/x 在区间 [1,4] 上的最大值和最小值。",
        ],
        "learning_advice": "建议先熟练掌握基本初等函数的求导公式，然后多做几道闭区间最值的练习题，特别是含参数的题型。每次做题时养成习惯：先求导→找驻点→列端点→全代入→比大小，按这个流程走就不容易漏步骤。",
    },
}
